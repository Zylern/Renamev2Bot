...
async def upload_mode(client: Client, msg: Message ) -> None:
    ...
    msg_str += "**1.) It will send renamed file in the same format it was before.**\n"
    if modee == UserDB.MODE_AS_DOCUMENT:
        msg_str += "➩ "
    
    msg_str += "**2.) It will send file as document.**\n"
    
    if modee == UserDB.MODE_AS_GMEDIA:
        msg_str += "➩ "
    
    msg_str += "**3.) It will send video as media**."
...
async def mode_callback(client, msg: MessageEntity):
    ...
    await msg.answer("Mode changed successfully.")
